...
export async function listAssessments({ page = 1, limit = 20, search = '', status = '' } = {}) {
  ...
  const response = await apiRequest(`/assessments?${params.toString()}`)
...
export async function getAssessment(id) {
  const response = await apiRequest(`/assessments/${id}`)
  return response.data
}
...
export async function deleteAssessment(id) {
  const response = await apiRequest(`/assessments/${id}`, { method: 'DELETE' })
  return response.data
}
